...
[IMAGE TITLE: Vector and Matrix Representations in a Neural Network]

**Image Description:**
- Left panel: A 4×4 pixel image shown as a grid of numbers, then "flattened" into a 16-element column vector
- Middle panel: A weight matrix W of shape (3×16) shown as a grid, with each row corresponding to one neuron
- Right panel: The result of multiplying W × x = z, producing a 3-element output vector
- Arrows connecting all three panels with labels "input vector", "weight matrix", "output"
- Below: Small diagram showing batch processing — multiple column vectors stacked into a data matrix X
...
[IMAGE TITLE: Matrix Multiplication in a Neural Network Forward Pass]

**Image Description:**
- Top row: Diagram showing a 3-neuron input layer and a 2-neuron hidden layer
- Below: The equivalent matrix operation: W (2×3) × x (3×1) = z (2×1)
- Colour-coded: Row 1 of W highlighted in blue, x highlighted in orange, C₁₁ result highlighted in green
- Annotation: "Each row of W = one neuron. Each element of z = one neuron's weighted sum."
- Inset: Batched version showing X (B×3) × Wᵀ (3×2) = Z (B×2), processing B examples simultaneously
...
[IMAGE TITLE: Dot Product Geometry and Neural Network Connection]

**Image Description:**
- Left panel: Two 2D vectors a and b drawn from the origin, with angle θ between them. Annotations showing ||a||, ||b||, and the projection of b onto a.
- Middle panel: Three cases side by side — parallel vectors (θ=0, large positive dot product), perpendicular vectors (θ=90, dot product=0), opposing vectors (θ=180, negative dot product)
- Right panel: A single neuron receiving 3 inputs x₁, x₂, x₃ with weights w₁, w₂, w₃. Shows z = w·x + b = w₁x₁ + w₂x₂ + w₃x₃ + b
- Bottom: Cosine similarity formula applied to two word embedding vectors for "king" and "queen"
...
[IMAGE TITLE: Linear Transformations — Geometric Visualisation]

**Image Description:**
- Four panels, each showing a 2D grid of points before and after a transformation
- Panel 1: Identity transformation (no change)
- Panel 2: Rotation by 45° — grid rotates while remaining a uniform grid
- Panel 3: Scaling — grid expands uniformly
- Panel 4: Shear transformation — grid slants (horizontal lines stay horizontal, vertical lines tilt)
- Each panel shows the transformation matrix W and the effect on basis vectors [1,0] and [0,1]
- Caption: "Every weight matrix in a neural network performs one of these transformations — or a combination"
...
[IMAGE TITLE: Eigenvectors and Eigenvalues — Visual Intuition]

**Image Description:**
- Left panel: A 2D ellipse transformation. Most vectors (shown as small blue arrows) rotate when multiplied by matrix A. Two red arrows (eigenvectors) only change length, not direction.
- Middle panel: Table showing eigenvalue λ > 1 (vector stretches), λ = 1 (vector unchanged), 0 < λ < 1 (vector shrinks), λ < 0 (vector flips direction)
- Right panel: PCA diagram. A cloud of 2D data points. Two arrows showing eigenvectors of the covariance matrix. Longer arrow = larger eigenvalue = direction of maximum variance. Labels: "PC1 (principal component 1)" and "PC2"
...
[IMAGE TITLE: Derivatives of Common Activation Functions]

**Image Description:**
- Three side-by-side plots, x-axis from -4 to 4
- Plot 1 (Sigmoid): Blue curve showing sigmoid function (S-shape from 0 to 1), orange dashed curve showing its derivative (bell shape peaking at 0.25 when x=0). Label: "Derivative squashes to near zero for large |x| — vanishing gradient"
- Plot 2 (ReLU): Blue curve (zero for x<0, linear for x>0), orange dashed curve (0 for x<0, flat 1 for x>0). Label: "Derivative is 1 for positive x — gradient flows cleanly"
- Plot 3 (Tanh): Blue curve (S-shape from -1 to 1), orange dashed curve (bell shape peaking at 1 when x=0). Label: "Better than sigmoid but still vanishes at extremes"
...
[IMAGE TITLE: Backpropagation as Chain Rule — Forward and Backward Pass]

**Image Description:**
- Top half: Forward pass. Left to right: input x, weight w₁, Layer 1 (z₁, a₁), weight w₂, Layer 2 (z₂, a₂), Loss L. Arrows pointing right. Values stored at each node.
- Bottom half: Backward pass. Right to left: ∂L/∂L=1, ∂L/∂a₂, ∂L/∂z₂, ∂L/∂w₂, ∂L/∂a₁, ∂L/∂z₁, ∂L/∂w₁. Red arrows pointing left. Each step multiplies the incoming gradient by the local derivative.
- Center: Mathematical annotation at each node showing the chain rule multiplication
- Caption: "Backpropagation: Forward pass stores values. Backward pass multiplies gradients. Chain rule connects them."
...
[IMAGE TITLE: Gradient Descent on a 3D Loss Landscape]

**Image Description:**
- A 3D surface plot showing a bowl-shaped loss landscape with a global minimum at the bottom
- A series of red dots showing the trajectory of gradient descent, spiralling down toward the minimum
- Arrows at each dot pointing in the direction of the negative gradient (downhill)
- Inset: The weight update equation w_new = w - η∇L with learning rate η labelled
- Three separate curves on the right showing different learning rates: too small (very slow convergence), too large (oscillates, diverges), just right (smooth descent)
...
[IMAGE TITLE: Probability Concepts in Neural Network Output]

**Image Description:**
- Left panel: A neural network classifier with softmax output. Output layer shows probabilities: [0.02, 0.05, 0.91, 0.01, 0.01] for 5 classes. Label: "Model outputs a probability distribution"
- Middle panel: Bernoulli distribution for binary classification (two bars at 0 and 1)
- Right panel: Categorical distribution for 10-class classification (10 bars, one tall bar at class 7 for a digit "7" input)
- Bottom: The cross-entropy loss formula: L = −Σᵢ yᵢ log(p̂ᵢ), connecting MLE to the loss function
...
[IMAGE TITLE: Gaussian Distribution and Deep Learning Applications]

**Image Description:**
- Left panel: Classic bell curve of N(0,1), x-axis from -4 to 4. Shaded regions showing ±1σ (68%), ±2σ (95%), ±3σ (99.7%)
- Middle panel: Three Gaussian curves overlaid, showing effect of changing μ (shifts the curve left/right) and σ (widens or narrows the curve)
- Right panel: 2D multivariate Gaussian shown as a contour plot (concentric ellipses). Arrows showing the eigenvectors of the covariance matrix Σ aligning with the principal axes of the ellipse
- Bottom row: Two application diagrams. Left: Weight histogram before/after batch normalisation (before: skewed distribution; after: bell curve centred near 0). Right: VAE latent space showing the N(0,I) prior and encoded data distribution
...
[IMAGE TITLE: Logarithm Behaviour and Cross-Entropy Loss]

**Image Description:**
- Left panel: Graph of y = log(x) for x from 0 to 2. x-axis labelled "predicted probability p̂". y-axis labelled "log(p̂)". Curve starts at -∞ at x=0, passes through 0 at x=1. Annotation: "Heavily penalises probabilities near 0"
- Right panel: Graph of binary cross-entropy loss L = −log(p̂) for y=1 (true label = 1). x-axis: predicted probability p̂ from 0 to 1. As p̂ → 1, loss → 0 (correct). As p̂ → 0, loss → ∞ (incorrect). Annotation: "Predicting 0 for a true positive → infinite loss"
- Bottom: Comparison table showing squared error vs cross-entropy for a wrong confident prediction — cross-entropy penalises far more strongly
...
[IMAGE TITLE: Softmax Function — Logits to Probabilities]

**Image Description:**
- Left: Bar chart showing raw logits z = [2.0, 1.0, 0.1, -0.5, 1.5] for 5 classes. Bars can be any height, including negative.
- Middle: Transformation diagram showing the two-step softmax: first exponentiate each logit (all positive now), then divide by sum (normalise to sum to 1)
- Right: Bar chart showing softmax probabilities for the same 5 classes. All bars between 0 and 1, summing to 1.
- Bottom row: Three bar charts showing the effect of temperature. T=0.1 (one bar near 1, others near 0), T=1 (standard softmax), T=10 (nearly flat/uniform distribution)
- Caption: "Softmax is the 'vote counter' of the neural network — it converts raw scores into a democratic probability distribution"
...
[IMAGE TITLE: Deep Learning Mathematics Mind Map]

**Image Description:**
- Central node: "Deep Learning Mathematics"
- Six main branches radiating outward
- Branch 1 (Linear Algebra): Vectors → Matrices → MatMul → Dot Product → Linear Transformations → Eigenvalues
- Branch 2 (Calculus): Derivatives → Partial Derivatives → Chain Rule → Gradients → Gradient Descent
- Branch 3 (Probability): Sample Space → Random Variables → MLE → Bayes' Theorem
- Branch 4 (Distributions): Gaussian → Mean/Variance → CLT → Weight Initialisation
- Branch 5 (Information Theory): Logarithms → Cross-Entropy → KL Divergence → Mutual Information
- Branch 6 (Output Layer): Logits → Softmax → Probabilities → Temperature Scaling
- Colour-coded: Blue = Linear Algebra, Red = Calculus, Green = Probability, Purple = Output
- Arrows between branches showing dependencies: "Chain Rule uses Partial Derivatives", "Cross-Entropy uses Log", etc.
