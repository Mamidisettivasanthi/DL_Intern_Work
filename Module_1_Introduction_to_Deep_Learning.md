...
[IMAGE PLACEHOLDER: The AI Landscape — Concentric Circles]

**Image Description:**
- Outermost circle labelled "Artificial Intelligence (AI)" in blue
- Middle circle labelled "Machine Learning (ML)" in green
- Innermost circle labelled "Deep Learning (DL)" in orange
- Small dot at the centre labelled "Generative AI / LLMs"
- Arrows pointing from outer to inner indicating increasing specialisation
- Timeline on the right showing AI (1950s), ML (1980s), DL (2010s)
...
[IMAGE PLACEHOLDER: AI vs ML vs DL Venn Diagram with Examples]

**Image Description:**
- Three nested rectangles
- Outer: "Artificial Intelligence" — examples: rule-based systems, expert systems, game-playing AI
- Middle: "Machine Learning" — examples: SVMs, Random Forests, Logistic Regression
- Inner: "Deep Learning" — examples: CNNs, RNNs, Transformers, GANs
- Each section has a representative icon (robot, chart, neural network)
...
[IMAGE PLACEHOLDER: Deep Learning Timeline — Horizontal Infographic]

**Image Description:**
- Horizontal timeline from 1943 to 2024
- Key milestones marked with dots and labels
- Colour-coded sections: Foundation Era (1943–1969), Dark Ages (1970–1985), Renaissance (1986–2011), Modern Era (2012–present)
- Icons for each milestone: neuron icon for McCulloch-Pitts, brain icon for perceptron, snowflake for AI winters, rocket for AlexNet, robot for GPT
- Brief description under each milestone
...
[IMAGE PLACEHOLDER: Single Artificial Neuron Diagram]

**Image Description:**
- Inputs x₁, x₂, x₃ shown as arrows entering the neuron from the left
- Each arrow labelled with weight w₁, w₂, w₃
- Bias b shown as a separate input with constant value 1
- Central circle representing the neuron showing the summation (Σ)
- Output arrow labelled f(z) leaving the neuron to the right
- A small box showing the activation function graph (sigmoid curve)
...
[IMAGE PLACEHOLDER: Multi-Layer Feedforward Neural Network]

**Image Description:**
- Four vertical columns of circles representing: Input Layer (4 neurons), Hidden Layer 1 (5 neurons), Hidden Layer 2 (5 neurons), Output Layer (3 neurons)
- All neurons in adjacent layers connected by arrows (fully connected)
- Input layer labelled with x₁, x₂, x₃, x₄
- Output layer labelled with probabilities (e.g., Cat: 0.8, Dog: 0.15, Bird: 0.05)
- Arrows pointing left to right showing forward pass direction
- Dashed backward arrows from output to input labelled "Backpropagation"
...
[IMAGE PLACEHOLDER: Training and Validation Loss Curves]

**Image Description:**
- X-axis: Number of Epochs (0 to 50)
- Y-axis: Loss value
- Blue curve: Training Loss (steadily decreasing)
- Orange curve: Validation Loss (decreases then starts increasing — showing overfitting point)
- Vertical dashed line at the optimal epoch before overfitting begins
- Region labels: "Underfitting" (early epochs), "Good Fit" (middle region), "Overfitting" (late epochs)
...
[IMAGE PLACEHOLDER: Framework Ecosystem Diagram]

**Image Description:**
- Central box: "Your Model (Python)"
- Arrows to three columns for TensorFlow, PyTorch, JAX
- Under TensorFlow: TF Serving, TF Lite, TF.js, Keras
- Under PyTorch: TorchServe, PyTorch Mobile, Lightning, Hugging Face
- Under JAX: Flax, Haiku, Optax, Orbax
- Bottom section: "Common Hardware" → GPU (NVIDIA CUDA), TPU (Google), CPU
...
[IMAGE PLACEHOLDER: Deep Learning Mind Map — Summary]

**Image Description:**
- Central node: "Deep Learning"
- Branch 1: "Foundations" → AI → ML → DL hierarchy
- Branch 2: "Neural Network" → Neuron → Layers → Activation Functions → Loss Functions
- Branch 3: "Training" → Backpropagation → Gradient Descent → Optimisers
- Branch 4: "Applications" → Vision → NLP → Healthcare → Robotics
- Branch 5: "Frameworks" → TensorFlow → PyTorch → JAX
- Branch 6: "Hardware" → GPU → TPU → Cloud
- Branch 7: "Challenges" → Overfitting → Interpretability → Data Hunger
